...
use BackToVendor\Doctrine\DBAL\Platforms\PostgreSQLPlatform;
use BackToVendor\Doctrine\DBAL\Schema\DefaultSchemaManagerFactory;
use BackToVendor\Doctrine\DBAL\Tools\DsnParser;
use BackToVendor\Symfony\Component\Lock\BlockingSharedLockStoreInterface;
use BackToVendor\Symfony\Component\Lock\BlockingStoreInterface;
...
class DoctrineDbalPostgreSqlStore implements BlockingSharedLockStoreInterface, BlockingStoreInterface
{
    ...
    public function waitAndSave(Key $key)
    {
        ...
        try {
            $this->conn->executeStatement($sql, ['key' => $this->getHashedKey($key)]);
            $lockAcquired = \true;
        } finally {
            if (!$lockAcquired) {
                $this->getInternalStore()->delete($key);

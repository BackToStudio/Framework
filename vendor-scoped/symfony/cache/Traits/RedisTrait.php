 ...
 */
namespace BackToVendor\Symfony\Component\Cache\Traits;

use BackToVendor\Predis\Command\Redis\UNLINK;
use BackToVendor\Predis\Connection\Aggregate\ClusterInterface;
use BackToVendor\Predis\Connection\Aggregate\RedisCluster;
use BackToVendor\Predis\Connection\Aggregate\ReplicationInterface;
use BackToVendor\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use BackToVendor\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
use BackToVendor\Predis\Response\ErrorInterface;
use BackToVendor\Predis\Response\Status;
use BackToVendor\Symfony\Component\Cache\Exception\CacheException;
use BackToVendor\Symfony\Component\Cache\Exception\InvalidArgumentException;
use BackToVendor\Symfony\Component\Cache\Marshaller\DefaultMarshaller;
use BackToVendor\Symfony\Component\Cache\Marshaller\MarshallerInterface;
/**
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redis instanceof \Redis && !$redis instanceof \RedisArray && !$redis instanceof \RedisCluster && !$redis instanceof \BackToVendor\Predis\ClientInterface && !$redis instanceof RedisProxy && !$redis instanceof RedisClusterProxy) {
        ...
        if ($redis instanceof \BackToVendor\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !class_exists(\BackToVendor\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !class_exists(\BackToVendor\Predis\Client::class) && !class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \BackToVendor\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !is_a($class, \BackToVendor\Predis\Client::class, \true) && !class_exists(\RedisSentinel::class)) {
        ...
        } elseif (is_a($class, \BackToVendor\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \BackToVendor\Predis\ClientInterface && ($this->redis->getConnection() instanceof ClusterInterface || $this->redis->getConnection() instanceof Predis2ClusterInterface)) {
    ...
    {
        if ($this->redis instanceof \BackToVendor\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = reset($hosts);
        if ($host instanceof \BackToVendor\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \BackToVendor\Predis\ClientInterface) {
            ...
            if (!version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \BackToVendor\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \BackToVendor\Predis\ClientInterface ? $host->scan($cursor ?? 0, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, ?object $redis = null): \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \BackToVendor\Predis\ClientInterface && ($redis->getConnection() instanceof RedisCluster || $redis->getConnection() instanceof Predis2RedisCluster)) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \BackToVendor\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \BackToVendor\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \BackToVendor\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \BackToVendor\Predis\ClientInterface) {
            ...
            if (($connection instanceof ClusterInterface || $connection instanceof Predis2ClusterInterface) && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \BackToVendor\Predis\Client($c);
                }
            }

...
use BackToVendor\Predis\Connection\Aggregate\PredisCluster;
...
use BackToVendor\Symfony\Component\Cache\CacheItem;
use BackToVendor\Symfony\Component\Cache\Exception\InvalidArgumentException;
use BackToVendor\Symfony\Component\Cache\Exception\LogicException;
use BackToVendor\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use BackToVendor\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use BackToVendor\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use BackToVendor\Symfony\Component\Cache\Traits\RedisClusterProxy;
use BackToVendor\Symfony\Component\Cache\Traits\RedisProxy;
use BackToVendor\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \BackToVendor\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use ($ids, $lua) {
            foreach ($ids as $id) {
                yield 'eval' => $this->redis instanceof \BackToVendor\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1];
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use ($tagIds, $lua) {
            ...
            foreach ($tagIds as $id) {
                yield 'eval' => $this->redis instanceof \BackToVendor\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1];
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
